...
start:		mov	pllcon,#0
		...
		lcall 	initsio			;init baudrategenerator (t3) op 9600 baud (pllcon wordt gebruikt)
...
uart_lees1:	cjne	a,#1h,uart_lees2	;counter op 1 => in buf2
		...
		inc	uart_in_counter
		sjmp	uart_trans
uart_lees2:	cjne	R0,#CR,uart_error	;CR => commando checken en uitvoeren

; <= COMMANDO's =>

; DIRECT BYTE NIET COMPAREN (enkel register of acc)		
uart_GH:	cjne	uart_in1,#'G',uart_OH	;commando GH check
		cjne	uart_in2,#'H',uart_error
		cpl	p2.6;
		; COM GH
uart_OH:	cjne	uart_in1,#'O',uart_error	;commando OH check
		cjne	uart_in2,#'H',uart_error
		cpl	p2.5;
		; COM OH
;<= afhandelen eindstates receiving =>	
uart_error:	; LED knipperen (PWM?) als : zet lichtjes terug uit
		cpl	p2.4;
		sjmp	uart_trans
